...
* ==========================================================
* PHASE 3: Two separate what-if sweeps
*   A) Change ONE objective coefficient: cost('h8')
*   B) Change ONE RHS value: capHour('h0')
* ==========================================================

Set t /t1*t21/;

Parameter
    coefVal(t)  "trial values for objective coefficient cost('h8')"
    rhsVal(t)   "trial values for RHS capHour('h0')"
    objCoef(t)  "optimal objective for coefficient sweep"
    objRhs(t)   "optimal objective for RHS sweep";

Scalar origCost8, origCapH0;

origCost8 = cost('h8');
origCapH0 = capHour('h0');

* --- A) Sweep objective coefficient: from 0.5*orig to 1.5*orig
coefVal(t) = (0.5 + (ord(t)-1)*(1.0/(card(t)-1))) * origCost8;
...
* --- B) Sweep RHS: from 0.6*orig to 1.4*orig (اگر اثر نداد بازه را بیشتر کن)
* --- B) Sweep ONE RHS: totShort('c1')  (total shortage cap for company 1)
Scalar origTotC1;
origTotC1 = totShort('c1');

rhsVal(t) = 0 + (ord(t)-1)*( (80-0)/(card(t)-1) );  

loop(t,
    totShort('c1') = rhsVal(t);
    Solve Telecom_MIP using MIP minimizing z;
    objRhs(t) = z.l;
);

totShort('c1') = origTotC1;


* --- Write TWO separate CSV files (easier to plot)
file f1 /sweep_coef.csv/; put f1;
put 't,cost_h8,obj',;
loop(t,
  put t.tl, ',', coefVal(t):0:6, ',', objCoef(t):0:6 /;
);
putclose f1;

file f2 /sweep_rhs.csv/; put f2;
put 't,totShort_c1,obj',;
loop(t,
  put t.tl, ',', rhsVal(t):0:6, ',', objRhs(t):0:6 /;
);
putclose f2;

display coefVal, objCoef, rhsVal, objRhs;
